...
use crate::data::albums as data;
use crate::error::AppError;
use crate::models::album::{AlbumResponse, CreateAlbumRequest};
...
pub async fn index(state: web::Data<AppState>) -> Result<HttpResponse, AppError> {
    let albums = data::list(&state.db).await?;
...
pub async fn show(
    ...
    match data::by_id(&state.db, id).await? {
        Some(album) => Ok(HttpResponse::Ok().json(album.to_response())),
...
pub async fn create(
    ...
    let album = data::create(&state.db, &body.name, release_date, now).await?;
